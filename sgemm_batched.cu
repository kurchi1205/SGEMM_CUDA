...
int main(int argc, char **argv) {
  ...
  for (int size : SIZE) {
    ...
    std::cout << "dimensions(m=n=k) " << m << ", batch size: " << bs << ", alpha: " << alpha
              << ", beta: " << beta << std::endl;

    float **Aarray = new float *[bs];
    float **Barray = new float *[bs];
    float **Carray = new float *[bs];
    float **CarrayRef = new float *[bs];

    for (int i = 0; i < bs; ++i) {
        Aarray[i] = dA + i * m * k;
        Barray[i] = dB + i * k * n;
        Carray[i] = dC + i * m * n;
        CarrayRef[i] = dC_ref + i * m * n;
    }

    float **d_Aarray, **d_Barray, **d_Carray, **d_CarrayRef;
    cudaMalloc(&d_Aarray, bs * sizeof(float *));
    cudaMalloc(&d_Barray, bs * sizeof(float *));
    cudaMalloc(&d_Carray, bs * sizeof(float *));
    cudaMalloc(&d_CarrayRef, bs * sizeof(float *));
    ...
    cudaMemcpy(d_CarrayRef, CarrayRef, bs * sizeof(float *), cudaMemcpyHostToDevice);

    // Verify the correctness of the calculation, and execute it once before the
    // kernel function timing to avoid cold start errors
    run_kernel_batched(13, bs, m, n, k, alpha, dA, dB, beta, dC_ref, d_Aarray, d_Barray, d_CarrayRef,
                handle); // cuBLAS
    run_kernel_batched(kernel_num, bs, m, n, k, alpha, dA, dB, beta, dC, d_Aarray, d_Barray, d_Carray,
                handle); // Executes the kernel, modifies the result matrix
    cudaCheck(cudaDeviceSynchronize());
    cudaCheck(cudaGetLastError()); // Check for async errors during kernel run
    
    cudaMemcpy(Carray, d_Carray, bs * sizeof(float *), cudaMemcpyDeviceToHost);
    cudaMemcpy(CarrayRef, d_CarrayRef, bs * sizeof(float *), cudaMemcpyDeviceToHost);

    for (int i = 0; i < bs; ++i) {
        cudaMemcpy(dC + i * m * n, Carray[i], sizeof(float) * m * n, cudaMemcpyDeviceToHost);
        cudaMemcpy(dC_ref + i * m * n, CarrayRef[i], sizeof(float) * m * n, cudaMemcpyDeviceToHost);
    }
    ...
    if (!verify_matrix(C_ref, C, m * n * bs)) {
      std::cout
    ...
    for (int j = 0; j < repeat_times; j++) {
      ...
      run_kernel_batched(kernel_num, bs, m, n, k, alpha, dA, dB, beta, dC, d_Aarray, d_Barray, d_Carray, handle);
    }
    cudaEventRecord(end);
